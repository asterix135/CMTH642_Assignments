...
The f-stastic is a good overall performance measure, balancing precision and
...
```{r cm base combo}
# complete confusion matrix for final model
...
on the training data, better than the 64% accuracy (at a 0.5 threshhold) from 
the original paper.  

The f-stats for the minority classes (3,4,7 and 8), however, are noticably
...
degredation in performance - essentially to the level of the original paper,
likely indicating some overfitting.

Importantly, our model is doing a very poor job at predicting minority classes; 
especially quality ratings of 3, 4, and 8, which have the weakest representation
in our data.
...
```{r reshape, cache=TRUE, warning=FALSE, message=FALSE}
...
```{r examine new}
hist(training$quality, col=rgb(1,0,0,0.5),
     main="Original vs. Reshaped Quality Distribution", 
     xlab="Quality Rating")
hist(new_training, col=rgb(0,0,1,0.5), add=T)
legend("topright",
       c("Original", "Reshaped"),
...
# **ADD SOME COMMENTARY ABOUT DISTRIBUTION DIFFERENCE**
...
returns the predictions for the final model and each of the intermdiate models. 
...
Psudocode for the SMOTE algorithm can be found in Chawla et al "SMOTE Synthetic 
