...
Portuguese red vinho verde wines, and to compare our results with those 
obtained by the original paper to work with the data.
...
oversamling algorithm to deal with class imbalance in the data.  The results
of the undersampling/SMOTE oversampling algorithm are contrasted with the
results of the original paper, as well as a much cruder approach to over/
under sampling.
...
derived (the median value from 3 or more judges), it's safe to say that these 
are not absolute values, but rather approximations.  As such, regression 
...
level in the original paper -- a difference of less than 0.5 would round to 
the correct value.
...
excellent wines.  For the purposes of this analysis, we define non-average-rated
wines as having a quality score of 3, 4, 7 or 8.
...
"Practical Machine Learning"^[Course notes available at: http://sux13.github.io/DataScienceSpCourseNotes/8_PREDMACHLEARN/Practical_Machine_Learning_Course_Notes.pdf, starting on p. 67].  This is essentially a stacking
algorithm.
...
model.  This is set through the trainControl function in the caret library.

## Normalizing Data

As per the original paper, we will be evaluating the feed-in models based on
standardized data.  This is done through the preProcess function in caret.
...
## **Say something about new distribution once plotted**

But more importantly, let's see if our reshaped data gives us a better model.
We will look at how the model works for both our original training data, and
how well it applies to the testing data set.

```{r new model training, cache=TRUE, warning=FALSE, message=FALSE}
...
```{r update and display final results comparison}
# confusion Matrix for model applied to original training data
...
f_stat <- NULL
for (i in c(1,2,5,6)) {
    newf <-  (2 * cm3$byClass[i,1] * cm3$byClass[i,3]) / 
...
f_stat <- NULL
for (i in c(1,2,5,6)) {
    newf <-  (2 * cm4$byClass[i,1] * cm4$byClass[i,3]) / 
...
# 'Hatchet Job' Under/Oversample

We've seen ** with the elegant approach **

As a final comparison, let's see what happens if we just take a very crude
approach to over/under sampling.

In this case, we're going to reduce the two majority classes by 25%, and 
increase each of the three minority classes by 1000% (using SMOTE).^[The code
for this is not echoed in the report, but I updated the populate function to
change the os_rate parameter to 10.  Updated code is in the original RMD file.]

```{r new populate, echo=FALSE}
populate <- function(class_rep, near_nbrs, k) {
    ...
    os_rate <- 10  # oversample rate (1 = 100%)
...
```{r hatchet reshaping}
# Split classes
class3 <- training[training$quality==3,]
class4 <- training[training$quality==4,]
class5 <- training[training$quality==5,]
class6 <- training[training$quality==6,]
class7 <- training[training$quality==7,]
class8 <- training[training$quality==8,]

# Oversample minority classes by 1000% using SMOTE
class3 <- rbind(class3, smote(class3, training, TRUE))
class4 <- rbind(class4, smote(class4, training, TRUE))
class8 <- rbind(class8, smote(class8, training, TRUE))

# Undersample majority classes by 25%
set.seed(75657)
class5 <- class5[sample(nrow(class5), floor(nrow(class5) * 0.75)),]
class6 <- class5[sample(nrow(class6), floor(nrow(class6) * 0.75)),]

# Class 7 remains in it original form

# Combine reshaped data into new training set
hatchet_train <- rbind(class3, class4, class5, class6, class7, class8)
```

```{r build hatchet model, cache=TRUE}
hatchet_train_pred <- ensemble_eval(hatchet_train, training)
```

```{r build hatchet test, cache=TRUE}
hatchet_test_pred <- ensemble_eval(hatchet_train, testing)
```

```{r evaluate hatchet}
# confusion Matrix for 'hatchet job' model applied to original training data
cm5 <- confusionMatrix(hatchet_train_pred$final, training$quality)
cm5

# confusion matrix for model applied to testing data
cm6 <- confusionMatrix(hatchet_test_pred$final, testing$quality)
cm6

model_summary <- rbind(model_summary, rep(NA, 3), rep(NA,3))
model_summary[6,1] <- 'hatchet_train'
model_summary[7,1] <- 'hatchet_test'
model_summary[6,2] <- cm5$overall[1]
model_summary[7,2] <- cm5$overall[2]
f_stat <- NULL
for (i in c(1,2,5,6)) {
    newf <-  (2 * cm5$byClass[i,1] * cm5$byClass[i,3]) / 
        (cm5$byClass[i,1] + cm5$byClass[i,3])
    if (is.nan(newf)) {newf <- 0}
    f_stat<- c(f_stat, newf)
}
model_summary[6,3] <- mean(f_stat)
f_stat <- NULL
for (i in c(1,2,5,6)) {
    newf <-  (2 * cm6$byClass[i,1] * cm6$byClass[i,3]) / 
        (cm6$byClass[i,1] + cm6$byClass[i,3])
    if (is.nan(newf)) {newf <- 0}
    f_stat<- c(f_stat, newf)
}
model_summary[6,3] <- mean(f_stat)
...
# Comparison of Results to Original Paper
...
Essentially **conclude something**
