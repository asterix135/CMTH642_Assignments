...
oversamling algorithm to deal with class imbalance in the data.  
...
```{r base component update table, echo=FALSE}
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[3,1] <-'base_multinom'
model_summary[3,2] <- cm_mn$overall[1]
model_summary[3,3] <- find_minor_f(cm_mn)

model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[4,1] <-'base_naive_bayes'
model_summary[4,2] <- cm_nb$overall[1]
model_summary[4,3] <- find_minor_f(cm_nb)

model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[5,1] <-'base_random_forest'
model_summary[5,2] <- cm_rf$overall[1]
model_summary[5,3] <- find_minor_f(cm_rf)

model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[6,1] <-'base_boosting'
model_summary[6,2] <- cm_gbf$overall[1]
model_summary[6,3] <- find_minor_f(cm_gbf)

model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[7,1] <-'base_linear_discriminant'
model_summary[7,2] <- cm_lda$overall[1]
model_summary[7,3] <- find_minor_f(cm_lda)
```
...
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[8,1] <- 'reshaped_ensemble'
model_summary[8,2] <- cm3$overall[1]
model_summary[8,3] <- find_minor_f(cm3)

cm_mn_r <- confusionMatrix(new_test_results$mn, testing$quality)
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[9,1] <- 'reshaped_multinom'
model_summary[9,2] <- cm_mn_r$overall[1]
model_summary[9,3] <- find_minor_f(cm_mn_r)

cm_nb_r <- confusionMatrix(new_test_results$nb, testing$quality)
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[10,1] <- 'reshaped_naive_bayes'
model_summary[10,2] <- cm_nb_r$overall[1]
model_summary[10,3] <- find_minor_f(cm_nb_r)

cm_rf_r <- confusionMatrix(new_test_results$rf, testing$quality)
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[11,1] <- 'reshaped_random_forest'
model_summary[11,2] <- cm_rf_r$overall[1]
model_summary[11,3] <- find_minor_f(cm_rf_r)

cm_gbf_r <- confusionMatrix(new_test_results$gbm, testing$quality)
model_summary <- rbind(model_summary, rep(NA, 3))
model_summary[12,1] <- 'reshaped_boosting'
model_summary[12,2] <- cm_gbf_r$overall[1]
model_summary[12,3] <- find_minor_f(cm_gbf_r)
...
By implementing some over/under sampling reshaping of the original data, we 
do in fact see an increase in accuracy on the testing set.  The performance
on the training data actually fell a bit, indicating a somewhat less overfit
model.

In addition, we're seeing an improved average f-score for our minority class
wines, with actually some prediction class 4., although still no correct
predictions in class 3 or 8.
...
At the end of the day, however, we achieved very similar results to the 
original paper with a significantly more complicated approach.
...
Part of the reason for this weaker performance was likely the mix
of models we incuded in our ensemble, as these models were not evaluated prior
to inclusion.
...
It reduced the overall accuracy of the model by about 10%, while boosting the
average f-stat for minority classes by about 20%.  Overall predictive ability
for minority classes was still rather weak even after this transformation.

Overall, it appears, from this one example, that if minority class prediction
is an important metric for model evaluation, then this kind of approach to
class rebalancing may be worthwhile.  If the main criterion is overall accuracy,
then rebalancing may not be worth the effort, as it will almost certainly
reduce accuracy.

## Comments about predicing wine quality based on chemical ratings
...
chemical analyis is really going to be able to provide a strongly-reliable
set of predictors of wine quality.

As such, it's unlikely that we'd be able to achieve a model that would be 
great at predicting consumer perceptions of quality without taking into 
account the social cues that factor into those ratings.
