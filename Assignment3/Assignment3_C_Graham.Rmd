...
normObj <- preProcess(training[,-9], method=c('center', 'scale'))
train_norm <- predict(normObj, training)
...
As per the original paper, we will be evaluating based on normalized data.
Luckily, we already did this earlier in the *Variable Relationships* section,
and will thus be referencing train_norm in the following code.

## Model creation

Since this is a fairly complicated model, it will be created as an R function.
This is particularly important for the iterations to follow.

```{r model code, message=FALSE}
ensemble_model <- function(train_set, y_col, fit_control) {
    # Input - training set, col number of y variable & trainControl object
    # Output - final model
    multi_pred <- predict(
        train(train_set[,y_col] ~ ., data = train_set, method = 'multinom',
