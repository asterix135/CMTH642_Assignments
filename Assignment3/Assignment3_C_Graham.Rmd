...
Finally, note that for the final model, we will be selecting the modal value
from the interim results (the simplest model possible - a voting model).
...
So this is somewhat unexpected in terms of final results.  Not suprprisingly, 
each of our individual models has an accuracy hovering around 60% (68% for the 
random forest model).  But when plugged into an ensemble model, our accuracy
rate is essentially 100%.  In fact, the only error is a 3-rated wine that
was incorrectly classified as an 8.

```{r cm base combo}
confusionMatrix(base_model[[1]])
```

This smells like overfitting.  So, let's see how the model compares to the test
set.  As a reminder, the methodology is as follows:

1. Apply the normalization model from the training set to the test data
2. Run the base models on the normalized training set, and use those predictions 
to build an interim result data frame.
3. Run the final ensemble model on the interim result data frame
4. Evaluate accuracy

We will set this up as a function in case it has to be run again.
...
evaluate_test <- function(model_list, test_set) {
    ...
    # 4. apply final model to interim data frame
    test_pred <- predict(model_list[[1]], ensemble_data)
    return(test_pred)
}
```
